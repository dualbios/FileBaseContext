...
public partial class FileBaseContextQueryExpression : Expression, IPrintableExpression
{
    ...
    private static readonly MethodInfo LeftJoinMethodInfo = typeof(FileBaseContextQueryExpression).GetTypeInfo()
    ...
    private FileBaseContextQueryExpression(
    ...
    public FileBaseContextQueryExpression(IEntityType entityType)
    {
        _valueBufferParameter = Parameter(typeof(ValueBuffer), "valueBuffer");
        ServerQueryExpression = new FileBaseContextTableExpression(entityType);
        ...
        if (_clientProjections.Count > 0)
        {
            for (var i = 0; i < _clientProjections.Count; i++)
            {
                ...
                switch (projection)
                {
                    ...
                    case FileBaseContextQueryExpression inMemoryQueryExpression:
                    {
    ...
    public virtual void ApplySetOperation(MethodInfo setOperationMethodInfo, FileBaseContextQueryExpression source2)
    {
    ...
    public virtual void ApplyDistinct()
    {
        ...
        else
        {
            ...
            {
                var projection = _clientProjections[i];
                if (projection is FileBaseContextQueryExpression)
                {
    ...
    {
        expressionPrinter.AppendLine(nameof(FileBaseContextQueryExpression) + ": ");
    ...
    private FileBaseContextQueryExpression Clone()
    {
        _cloningExpressionVisitor ??= new CloningExpressionVisitor();

        return (FileBaseContextQueryExpression)_cloningExpressionVisitor.Visit(this);
    }
